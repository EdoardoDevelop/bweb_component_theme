	...
	DrSlider.prototype = {
		...
		_animate: function(selector, from, to, duration, delay, callback, jQueryAnimation){
			...
			if(event_end !== false){
				...
				setTimeout(function(){
                    var easeP = "cubic-bezier(0.170, 0.485, 0.130, 0.955)";
					var transition = {
						'-webkit-transition': 'all '+duration+'ms '+easeP+' '+delay+'ms',
						'-moz-transition': 'all '+duration+'ms '+easeP+' '+delay+'ms',
						'-o-transition': 'all '+duration+'ms '+easeP+' '+delay+'ms',
                        'transition': 'all '+duration+'ms '+easeP+' '+delay+'ms',
		...
		_showAnimation: function($element, callback, callback_ani){
			...
			this._transition($element, transition, function(){
				...
				that._playAnimation(arr_img_element,$element.data("startAll"), function(){
		...
		_transition_prepare: function($element){
			...
			if($element.data('lazy-background') && $element.children('.lazy-background').length == 0){
				/*var html = '<div src="'+$element.data('lazy-background')+'" class="lazy-background"></div>';
				$(html).appendTo($element).css({
					'position': 'absolute',
					'background-image':'url('+$element.data('lazy-background')+')',
					'background-position': $element.data('imagepos'),
			...
			$element.find('[lazy-background]').each(function(){
		...
		_playAnimation: function(arr_img_element, startAll, callback){
			var that = this;
			...
			switch($img_element.data('effect')){
				case 'fadein':
					this._animate(
						...
						function(){
							if(!startAll){
								if(arr_img_element.length > 0) that._playAnimation(arr_img_element, startAll, callback);
								else callback();
							}
						}
					);
